...
namespace fc_light
{
   ...
   {
      return  my->file + ":" + fc_light::to_string(my->line) + " " + my->method;
   ...
   void from_variant( const variant& v, log_level& e )
   {
      try 
      {
        ...
        else FC_LIGHT_THROW_EXCEPTION( bad_cast_exception, "Failed to cast from Variant to log_level" );
      } FC_LIGHT_RETHROW_EXCEPTIONS( error,
