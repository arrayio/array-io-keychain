...
#include <mutex>
#include <ethereum/core/Address.h>
#include <ethereum/core/Common.h>
...
void unpack( Stream& s, dev::Public& pk)
{
  std::vector<char> ser;
  fc::raw::unpack(s,ser);
  dev::bytesRef ser_ref(reinterpret_cast<byte*>(ser.data()), ser.size());
  pk = dev::Public(ser_ref);
}

template<typename Stream>
void pack( Stream& s, const dev::Public& pk)
{
  dev::bytesConstRef key_ref(pk.data(), pk.size);
	std::vector<char> key_vec(pk.size);
	std::copy(key_ref.begin(), key_ref.end(), key_vec.begin());
  fc::raw::pack( s, key_vec );
}

template<typename Stream>
void unpack( Stream& s, dev::Secret& pk)
{
	std::vector<char> ser;
	fc::raw::unpack(s,ser);
	dev::bytesRef ser_ref(reinterpret_cast<byte*>(ser.data()), ser.size());
	pk = dev::Secret(ser_ref);
}

template<typename Stream>
void pack( Stream& s, const dev::Secret& pk)
{
	dev::bytesConstRef key_ref(pk.data(), pk.size);
	std::vector<char> key_vec(pk.size);
	std::copy(key_ref.begin(), key_ref.end(), key_vec.begin());
