...
namespace fc_keychain  {
  ...
  int64_t    to_int64( const fc_keychain::string& i )
  {
    ...
    {
      FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Couldn't parse int64_t" );
    }
    FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "${i} => int64_t", ("i",i) )
  }
  ...
  { try {
    ...
    {
      FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Couldn't parse uint64_t" );
    }
    FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "${i} => uint64_t", ("i",i) )
  } FC_KEYCHAIN_CAPTURE_AND_RETHROW( (i) ) }
  ...
  double     to_double( const fc_keychain::string& i)
  {
    ...
    {
      FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Couldn't parse double" );
    }
    FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "${i} => double", ("i",i) )
  }
  ...
  uint64_t parse_size( const string& s )
  {
     try
     {
        ...
        FC_KEYCHAIN_ASSERT( n - suffix_start <= 3 );
        ...
        {
           FC_KEYCHAIN_ASSERT( (s[suffix_start+1] == 'b') || (s[suffix_start+1] == 'B') );
           thousand = 1000;
        }
        else if( suffix_start == n-3 )
        {
           FC_KEYCHAIN_ASSERT( (s[suffix_start+1] == 'i') || (s[suffix_start+1] == 'I') );
           FC_KEYCHAIN_ASSERT( (s[suffix_start+2] == 'b') || (s[suffix_start+2] == 'B') );
        }
        switch( s[suffix_start] )
        {
           ...
           case 'K':
              ...
              break;
           default:
              FC_KEYCHAIN_ASSERT( false );
        }
        return u*m;
     }
     catch( const fc_keychain::exception& e )
     {
        FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Couldn't parse size" );
     }
  }
