...
# pragma warning(disable:4522) /* multiple assignment operators */
   ...
   *  fc_keychain::optional adds less than 400.
   */
  template<typename T>
  class optional
  {
    public:
      ...
      :_valid(false) 
      {
        if( o._valid ) new (ptr()) T( fc_keychain::move(*o) );
        _valid = o._valid;
        o.reset();
      }
      ...
      :_valid(true) 
      {
        new ((char*)ptr()) T( fc_keychain::forward<U>(u) );
      }
      ...
      {
        reset();
        new (ptr()) T( fc_keychain::forward<U>(u) );
        _valid = true;
        return *this;
      }
      ...
      optional& operator=( optional<U>&& o ) 
      {
        if (this != &o) 
        {
          if( _valid && o._valid ) 
          {
            ref() = fc_keychain::move(*o);
            o.reset();
          } else if ( !_valid && o._valid ) {
            *this = fc_keychain::move(*o);
          } else if (_valid) {
            reset();
          }
        }
