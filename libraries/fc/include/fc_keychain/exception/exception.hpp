...
namespace fc_keychain
{
    ...
    *
    *  @see FC_KEYCHAIN_THROW_EXCEPTION
    *  @see FC_KEYCHAIN_RETHROW_EXCEPTION
    *  @see FC_KEYCHAIN_RETHROW_EXCEPTIONS
    */
   class exception
   {
      public:
...
#define FC_KEYCHAIN_REGISTER_EXCEPTION(r, unused, base) \
   fc_keychain::exception_factory::instance().register_exception<base>();

#define FC_KEYCHAIN_REGISTER_EXCEPTIONS( SEQ )\
     \
   static bool exception_init = []()->bool{ \
    BOOST_PP_SEQ_FOR_EACH( FC_KEYCHAIN_REGISTER_EXCEPTION, v, SEQ )  \
      return true; \
   }();  \


#define FC_KEYCHAIN_DECLARE_DERIVED_EXCEPTION( TYPE, BASE, CODE, WHAT ) \
  ...
  #define FC_KEYCHAIN_DECLARE_EXCEPTION( TYPE, CODE, WHAT ) \
      FC_KEYCHAIN_DECLARE_DERIVED_EXCEPTION( TYPE, fc_keychain::exception, CODE, WHAT )

  FC_KEYCHAIN_DECLARE_EXCEPTION( timeout_exception, timeout_exception_code, "Timeout" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( file_not_found_exception, file_not_found_exception_code, "File Not Found" );
  /**
   * @brief report's parse errors
   */
  FC_KEYCHAIN_DECLARE_EXCEPTION( parse_error_exception, parse_error_exception_code, "Parse Error" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( invalid_arg_exception, invalid_arg_exception_code, "Invalid Argument" );
  /**
   * @brief reports when a key, guid, or other item is not found.
   */
  FC_KEYCHAIN_DECLARE_EXCEPTION( key_not_found_exception, key_not_found_exception_code, "Key Not Found" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( bad_cast_exception, bad_cast_exception_code, "Bad Cast" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( out_of_range_exception, out_of_range_exception_code, "Out of Range" );

  /** @brief if an operation is unsupported or not valid this may be thrown */
  FC_KEYCHAIN_DECLARE_EXCEPTION( invalid_operation_exception,
                        invalid_operation_exception_code,
                        "Invalid Operation" );
  /** @brief if an host name can not be resolved this may be thrown */
  FC_KEYCHAIN_DECLARE_EXCEPTION( unknown_host_exception,
   ...
   */
  FC_KEYCHAIN_DECLARE_EXCEPTION( canceled_exception, canceled_exception_code, "Canceled" );
  /**
   *  @brief used inplace of assert() to report violations of pre conditions.
   */
  FC_KEYCHAIN_DECLARE_EXCEPTION( assert_exception, assert_exception_code, "Assert Exception" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( eof_exception, eof_exception_code, "End Of File" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( null_optional, null_optional_code, "null optional" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( aes_exception, aes_error_code, "AES error" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( overflow_exception, overflow_code, "Integer Overflow" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( underflow_exception, underflow_code, "Integer Underflow" );
  FC_KEYCHAIN_DECLARE_EXCEPTION( divide_by_zero_exception, divide_by_zero_code, "Integer Divide By Zero" );
  //  /* defined near assert_evaluator */ FC_KEYCHAIN_DECLARE_EXCEPTION( transaction_dependency_exception, transaction_dependency_code, "Missing Transaction Dependency" );
 ...
 */
#define FC_KEYCHAIN_EXPAND_MACRO( x ) x
/**
 *  @brief Checks a condition and throws an assert_exception if the test is FALSE
 */
#define FC_KEYCHAIN_ASSERT( TEST, ... ) \
  FC_KEYCHAIN_EXPAND_MACRO( \
    FC_KEYCHAIN_MULTILINE_MACRO_BEGIN \
      if( UNLIKELY(!(TEST)) ) \
      {                                                                      \
        if( fc_keychain::enable_record_assert_trip )                                  \
           fc_keychain::record_assert_trip( __FILE__, __LINE__, #TEST );              \
        FC_KEYCHAIN_THROW_EXCEPTION( fc_keychain::assert_exception, #TEST ": "  __VA_ARGS__ ); \
      }                                                                      \
    FC_KEYCHAIN_MULTILINE_MACRO_END \
  )

#define FC_KEYCHAIN_CAPTURE_AND_THROW( EXCEPTION_TYPE, ... ) \
  FC_KEYCHAIN_MULTILINE_MACRO_BEGIN \
    throw EXCEPTION_TYPE( FC_KEYCHAIN_LOG_MESSAGE( error, "", FC_KEYCHAIN_FORMAT_ARG_PARAMS(__VA_ARGS__) ) ); \
  FC_KEYCHAIN_MULTILINE_MACRO_END

//#define FC_KEYCHAIN_THROW( FORMAT, ... )
// FC_KEYCHAIN_INDIRECT_EXPAND workas around a bug in Visual C++ variadic macro processing that prevents it
// from separating __VA_ARGS__ into separate tokens
#define FC_KEYCHAIN_INDIRECT_EXPAND(MACRO, ARGS) MACRO ARGS
#define FC_KEYCHAIN_THROW(  ... ) \
  FC_KEYCHAIN_MULTILINE_MACRO_BEGIN \
    throw fc_keychain::exception( FC_KEYCHAIN_INDIRECT_EXPAND(FC_KEYCHAIN_LOG_MESSAGE, ( error, __VA_ARGS__ )) );  \
  FC_KEYCHAIN_MULTILINE_MACRO_END

#define FC_KEYCHAIN_EXCEPTION( EXCEPTION_TYPE, FORMAT, ... ) \
    EXCEPTION_TYPE( FC_KEYCHAIN_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) )
/**
 *  @def FC_KEYCHAIN_THROW_EXCEPTION( EXCEPTION, FORMAT, ... )
 *  @param EXCEPTION a class in the Phoenix::Athena::API namespace that inherits
 *  @param format - a const char* string with "${keys}"
 */
#define FC_KEYCHAIN_THROW_EXCEPTION( EXCEPTION, FORMAT, ... ) \
  FC_KEYCHAIN_MULTILINE_MACRO_BEGIN \
    throw EXCEPTION( FC_KEYCHAIN_LOG_MESSAGE( error, FORMAT, __VA_ARGS__ ) ); \
  FC_KEYCHAIN_MULTILINE_MACRO_END


/**
 *  @def FC_KEYCHAIN_RETHROW_EXCEPTION(ER,LOG_LEVEL,FORMAT,...)
 *  @brief Appends a log_message to the exception ER and rethrows it.
 */
#define FC_KEYCHAIN_RETHROW_EXCEPTION( ER, LOG_LEVEL, FORMAT, ... ) \
  FC_KEYCHAIN_MULTILINE_MACRO_BEGIN \
    ER.append_log( FC_KEYCHAIN_LOG_MESSAGE( LOG_LEVEL, FORMAT, __VA_ARGS__ ) ); \
    throw; \
  FC_KEYCHAIN_MULTILINE_MACRO_END

#define FC_KEYCHAIN_LOG_AND_RETHROW( )  \
   catch( fc_keychain::exception& er ) { \
      wlog( "${details}", ("details",er.to_detail_string()) ); \
      FC_KEYCHAIN_RETHROW_EXCEPTION( er, warn, "rethrow" ); \
   } catch( const std::exception& e ) {  \
      fc_keychain::exception fce( \
                FC_KEYCHAIN_LOG_MESSAGE( warn, "rethrow ${what}: ", ("what",e.what())), \
      ...
      fc_keychain::unhandled_exception e( \
                FC_KEYCHAIN_LOG_MESSAGE( warn, "rethrow"), \
                std::current_exception() ); \
      wlog( "${details}", ("details",e.to_detail_string()) ); \
      throw e; \
   }

#define FC_KEYCHAIN_CAPTURE_LOG_AND_RETHROW( ... )  \
   catch( fc_keychain::exception& er ) { \
      wlog( "${details}", ("details",er.to_detail_string()) ); \
      wdump( __VA_ARGS__ ); \
      FC_KEYCHAIN_RETHROW_EXCEPTION( er, warn, "rethrow", FC_KEYCHAIN_FORMAT_ARG_PARAMS(__VA_ARGS__) ); \
   } catch( const std::exception& e ) {  \
      fc_keychain::exception fce( \
                FC_KEYCHAIN_LOG_MESSAGE( warn, "rethrow ${what}: ", FC_KEYCHAIN_FORMAT_ARG_PARAMS( __VA_ARGS__ )("what",e.what())), \
                fc_keychain::std_exception_code,\
                typeid(e).name(), \
                e.what() ) ; \
      wlog( "${details}", ("details",fce.to_detail_string()) ); \
      ...
      fc_keychain::unhandled_exception e( \
                FC_KEYCHAIN_LOG_MESSAGE( warn, "rethrow", FC_KEYCHAIN_FORMAT_ARG_PARAMS( __VA_ARGS__) ), \
                std::current_exception() ); \
      wlog( "${details}", ("details",e.to_detail_string()) ); \
      wdump( __VA_ARGS__ ); \
      throw e; \
...
#define FC_KEYCHAIN_CAPTURE_AND_LOG( ... )  \
   catch( fc_keychain::exception& er ) { \
      wlog( "${details}", ("details",er.to_detail_string()) ); \
      wdump( __VA_ARGS__ ); \
   } catch( const std::exception& e ) {  \
...
/**
 *  @def FC_KEYCHAIN_RETHROW_EXCEPTIONS(LOG_LEVEL,FORMAT,...)
 *  @brief  Catchs all exception's, std::exceptions, and ... and rethrows them after
 *   appending the provided log message.
 */
#define FC_KEYCHAIN_RETHROW_EXCEPTIONS( LOG_LEVEL, FORMAT, ... ) \
   catch( fc_keychain::exception& er ) { \
      FC_KEYCHAIN_RETHROW_EXCEPTION( er, LOG_LEVEL, FORMAT, __VA_ARGS__ ); \
   } catch( const std::exception& e ) {  \
      fc_keychain::exception fce( \
                FC_KEYCHAIN_LOG_MESSAGE( LOG_LEVEL, "${what}: " FORMAT,__VA_ARGS__("what",e.what())), \
                fc_keychain::std_exception_code,\
                typeid(e).name(), \
                e.what() ) ; throw fce;\
   } catch( ... ) {  \
      throw fc_keychain::unhandled_exception( \
                FC_KEYCHAIN_LOG_MESSAGE( LOG_LEVEL, FORMAT,__VA_ARGS__), \
                std::current_exception() ); \
   }

#define FC_KEYCHAIN_CAPTURE_AND_RETHROW( ... ) \
   catch( fc_keychain::exception& er ) { \
      FC_KEYCHAIN_RETHROW_EXCEPTION( er, warn, "", FC_KEYCHAIN_FORMAT_ARG_PARAMS(__VA_ARGS__) ); \
   } catch( const std::exception& e ) {  \
      fc_keychain::exception fce( \
                FC_KEYCHAIN_LOG_MESSAGE( warn, "${what}: ",FC_KEYCHAIN_FORMAT_ARG_PARAMS(__VA_ARGS__)("what",e.what())), \
                fc_keychain::std_exception_code,\
                typeid(e).name(), \
                e.what() ) ; throw fce;\
   } catch( ... ) {  \
      throw fc_keychain::unhandled_exception( \
                FC_KEYCHAIN_LOG_MESSAGE( warn, "",FC_KEYCHAIN_FORMAT_ARG_PARAMS(__VA_ARGS__)), \
                std::current_exception() ); \
   }
