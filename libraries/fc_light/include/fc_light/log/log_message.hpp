...
namespace fc_light
{
   ...
   class log_context 
   {
      public:
        ...
        void          append_context( const fc_light::string& c );
   ...
   class log_message
   {
      public:
         log_message();
         /**
          *  @param ctx - generally provided using the FC_LIGHT_LOG_CONTEXT(LEVEL) macro
          */
...
FC_LIGHT_REFLECT_TYPENAME( fc_light::log_message );
...
/**
 * @def FC_LIGHT_LOG_CONTEXT(LOG_LEVEL)
 * @brief Automatically captures the File, Line, and Method names and passes them to
 *        the constructor of fc_light::log_context along with LOG_LEVEL
 * @param LOG_LEVEL - a valid log_level::Enum name.
 */
#define FC_LIGHT_LOG_CONTEXT(LOG_LEVEL) \
   fc_light::log_context( fc_light::log_level::LOG_LEVEL, __FILE__, __LINE__, __func__ )
   
/**
 * @def FC_LIGHT_LOG_MESSAGE(LOG_LEVEL,FORMAT,...)
 *
 ...
 */
#define FC_LIGHT_LOG_MESSAGE( LOG_LEVEL, FORMAT, ... ) \
   fc_light::log_message( FC_LIGHT_LOG_CONTEXT(LOG_LEVEL), FORMAT, fc_light::mutable_variant_object()__VA_ARGS__ )
