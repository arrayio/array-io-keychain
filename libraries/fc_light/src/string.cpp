...
namespace fc_light  {
  ...
  int64_t    to_int64( const fc_light::string& i )
  {
    ...
    {
      FC_LIGHT_THROW_EXCEPTION( parse_error_exception, "Couldn't parse int64_t" );
    }
    FC_LIGHT_RETHROW_EXCEPTIONS( warn, "${i} => int64_t", ("i",i) )
  }
  ...
  { try {
    ...
    {
      FC_LIGHT_THROW_EXCEPTION( parse_error_exception, "Couldn't parse uint64_t" );
    }
    FC_LIGHT_RETHROW_EXCEPTIONS( warn, "${i} => uint64_t", ("i",i) )
  } FC_LIGHT_CAPTURE_AND_RETHROW( (i) ) }
  ...
  double     to_double( const fc_light::string& i)
  {
    ...
    {
      FC_LIGHT_THROW_EXCEPTION( parse_error_exception, "Couldn't parse double" );
    }
    FC_LIGHT_RETHROW_EXCEPTIONS( warn, "${i} => double", ("i",i) )
  }
  ...
  uint64_t parse_size( const string& s )
  {
     try
     {
        ...
        FC_LIGHT_ASSERT( n - suffix_start <= 3 );
        ...
        {
           FC_LIGHT_ASSERT( (s[suffix_start+1] == 'b') || (s[suffix_start+1] == 'B') );
           thousand = 1000;
        }
        else if( suffix_start == n-3 )
        {
           FC_LIGHT_ASSERT( (s[suffix_start+1] == 'i') || (s[suffix_start+1] == 'I') );
           FC_LIGHT_ASSERT( (s[suffix_start+2] == 'b') || (s[suffix_start+2] == 'B') );
        }
     ...
     {
        FC_LIGHT_THROW_EXCEPTION( parse_error_exception, "Couldn't parse size" );
     }
  }
