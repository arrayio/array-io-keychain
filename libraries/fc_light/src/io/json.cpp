...
namespace fc_light
{
    ...
    template<typename T> fc_light::string stringFromStream( T& in );
    template<typename T> bool skip_white_space( T& in );
    template<typename T> fc_light::string stringFromToken( T& in );
...
#include <fc_light/io/json_relaxed.hpp>
...
namespace fc_light
{
   ...
   char parseEscape( T& in )
   {
      if( in.peek() == '\\' )
      {
         ...
         } FC_LIGHT_RETHROW_EXCEPTIONS( info, "Stream ended with '\\'" );
      }
         ...
         while( true )
         {
            ...
            switch( c = in.peek() )
            {
               case '\\':
                  ...
                  break;
               case 0x04:
                  FC_LIGHT_THROW_EXCEPTION( parse_error_exception, "EOF before closing '\"' in string '${token}'",
                                                   ("token", token.str() ) );
               case '"':
                  in.get();
                  return token.str();
   ...
   variant number_from_stream( T& in )
   {
      ...
      try
      {
        ...
        while((c = in.peek()) && !done)
        {
          ...
          switch( c )
          {
              case '.':
                 if (dot)
                    FC_LIGHT_THROW_EXCEPTION(parse_error_exception, "Can't parse a number with two decimal places");
                 dot = true;
              case '0':
              case '1':
              case '2':
      ...
      catch (fc_light::eof_exception&)
      {
      }
      catch (const std::ios_base::failure&)
      {
      }
      fc_light::string str = ss.str();
      if (str == "-." || str == ".") // check the obviously wrong things we could have encountered
        FC_LIGHT_THROW_EXCEPTION(parse_error_exception, "Can't parse token \"${token}\" as a JSON numeric constant", ("token", str));
      if( dot )
   ...
   variant token_from_stream( T& in )
   {
      ...
      else
      {
        ...
        {
          if (str.empty())
            FC_LIGHT_THROW_EXCEPTION( parse_error_exception, "Unexpected EOF" );
          else
            return str;
        }
        else
   ...
   void check_string_depth( const string& utf8_str  )
   {
      ...
      for( auto c : utf8_str )
      {
         ...
         FC_LIGHT_ASSERT( open_object < 100 && open_array < 100, "object graph too deep", ("object depth",open_object)("array depth", open_array) );
      }
   }
   ...
   } FC_LIGHT_RETHROW_EXCEPTIONS( warn, "", ("str",utf8_str) ) }
   ...
   */
   ...
   fc_light::string json::to_pretty_string( const variant& v, output_formatting format /* = stringify_large_ints_and_doubles */ )
   {
	   return pretty_print(to_string(v, format), 2);
   }

   void json::save_to_file( const variant& v, const fc_light::path& fi, bool pretty, output_formatting format /* = stringify_large_ints_and_doubles */ )
   {
      if( pretty )
      {
        auto str = json::to_pretty_string( v, format );
        fc_light::ofstream o(fi);
        o.write( str.c_str(), str.size() );
      }
      else
      {
       fc_light::ofstream o(fi);
       fc_light::to_stream( o, v, format );
      }
   }
   variant json::from_file( const fc_light::path& p, parse_type ptype )
   {
      //auto tmp = std::make_shared<fc_light::ifstream>( p, ifstream::binary );
...
/*   variant json::from_stream( buffered_istream& in, parse_type ptype )
   {
      ...
      {
          case legacy_parser:
              return variant_from_stream<fc_light::buffered_istream, legacy_parser>( in );
          case legacy_parser_with_string_doubles:
              return variant_from_stream<fc_light::buffered_istream, legacy_parser_with_string_doubles>( in );
          case strict_parser:
