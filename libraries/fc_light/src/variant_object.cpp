...
namespace fc_light
{
   ...
   variant_object::entry::entry( string k, variant v ) : _key(fc_light::move(k)),_value(fc_light::move(v)) {}
   variant_object::entry::entry( entry&& e ) : _key(fc_light::move(e._key)),_value(fc_light::move(e._value)) {}
   ...
   {
      fc_light_swap( _key, e._key );
      fc_light_swap( _value, e._value );
      return *this;
   }
   ...
   const variant& variant_object::operator[]( const char* key )const
   {
      ...
      FC_LIGHT_THROW_EXCEPTION( key_not_found_exception, "Key ${key}", ("key",key) );
   }
   ...
   variant_object::variant_object( string key, variant val )
      : _key_value(std::make_shared<std::vector<entry>>())
   {
       //_key_value->push_back(entry(fc_light::move(key), fc_light::move(val)));
       _key_value->emplace_back(entry(fc_light::move(key), fc_light::move(val)));
   }
   ...
   : _key_value( fc_light::move(obj._key_value) )
   {
   ...
   variant_object& variant_object::operator=( variant_object&& obj )
   {
      if (this != &obj)
      {
         fc_light_swap(_key_value, obj._key_value );
         assert( _key_value != nullptr );
      }
      return *this;
   }
   ...
   mutable_variant_object& mutable_variant_object::set( string key, variant var )
   {
      ...
      if( itr != end() )
      {
         itr->set( fc_light::move(var) );
      }
      else
      {
         _key_value->push_back( entry( fc_light::move(key), fc_light::move(var) ) );
      }
      return *this;
   }
