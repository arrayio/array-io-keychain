...
namespace fc_light {
  ...
  void create_directories( const path& p ) { 
    try {
      ...
      FC_LIGHT_THROW( "Unable to create directories ${path}", ("path", p )("inner", fc_light::except_str() ) );
    }
  }
  ...
  {
    try {
      FC_LIGHT_ASSERT( is_directory( p ) );
      ...
      FC_LIGHT_THROW( "Unable to calculate size of directory ${path}", ("path", p )("inner", fc_light::except_str() ) );
    }
  }
  ...
  void copy( const path& f, const path& t ) { 
     try {
     	...
     	FC_LIGHT_THROW( "Copy from ${srcfile} to ${dstfile} failed because ${reason}",
	         ("srcfile",f)("dstfile",t)("reason",e.what() ) );
     } catch ( ... ) {
     	FC_LIGHT_THROW( "Copy from ${srcfile} to ${dstfile} failed",
  ...
  { 
    ...
    {
      FC_LIGHT_THROW( "Resize file '${f}' to size ${s} failed: ${reason}",
  ...
  void chmod( const path& p, int perm )
  {
    ...
    if( result != 0 )
        FC_LIGHT_THROW( "chmod operation failed on ${p}", ("p",p) );
#endif
    return;
  }
  ...
  void rename( const path& f, const path& t ) { 
     try {
         ...
         try{
             ...
             FC_LIGHT_THROW( "Rename from ${srcfile} to ${dstfile} failed because ${reason}",
                     ("srcfile",f)("dstfile",t)("reason",e.what() ) );
         }
     } catch ( ... ) {
     	FC_LIGHT_THROW( "Rename from ${srcfile} to ${dstfile} failed",
	         ("srcfile",f)("dstfile",t)("inner", fc_light::except_str() ) );
     }
  }
  void create_hard_link( const path& f, const path& t ) { 
     try {
        boost::filesystem::create_hard_link( f, t ); 
         ...
         FC_LIGHT_THROW( "Unable to create hard link from '${from}' to '${to}'",
  ...
  bool remove( const path& f ) { 
     try {
        return boost::filesystem::remove( f ); 
         ...
         FC_LIGHT_THROW( "Unable to remove '${path}'", ( "path", f )("exception", fc_light::except_str() ) );
     }
  }
  fc_light::path canonical( const fc_light::path& p ) {
     try {
        return boost::filesystem::canonical(p); 
         ...
         FC_LIGHT_THROW( "Unable to resolve path '${path}'", ( "path", p )("exception", fc_light::except_str() ) );
     }
  }
  ...
  fc_light::path make_relative(const fc_light::path& from, const fc_light::path& to) {
   ...
   : temp_file_base(p / fc_light::unique_path())
   {
      ...
      {
         FC_LIGHT_THROW( "Name collision: ${path}", ("path", _path->string()) );
      }
      if (create)
      {
   ...
   const fc_light::path& temp_file_base::path() const
   {
      if (!_path)
      {
         FC_LIGHT_THROW( "Temporary directory has been released." );
      }
      return *_path;
   }
   ...
   const fc_light::path& home_path()
   {
      static fc_light::path p = []()
      {
          ...
          if (!success)
            FC_LIGHT_ASSERT(false, "Unable to get the user profile directory");
          ...
          if( nullptr == home )
          {
             ...
             FC_LIGHT_ASSERT( home != nullptr, "The HOME environment variable is not set" );
          }
   ...
   const fc_light::path& app_path()
   {
         ...
         static fc_light::path appdir = [](){
           ...
           if (!SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_APPDATA | CSIDL_FLAG_CREATE, NULL, 0, app_data_dir)))
             FC_LIGHT_ASSERT(false, "Unable to get the current AppData directory");
