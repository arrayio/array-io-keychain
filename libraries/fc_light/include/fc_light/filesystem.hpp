...
namespace fc_light {
   ...
   *  wrapped in an fc_light::error_report and the additional helper method fc_light::path::windows_string(),
   *  can be used to calculate paths intended for systems different than the host.
   *
   *  @note Serializes to a fc_light::value() as the result of generic_string()
   */
  class path {
    public:
      path();
      ...
      path& operator /=( const fc_light::path& );
      friend path operator /( const fc_light::path& p, const fc_light::path& );
      friend bool operator ==( const fc_light::path& p, const fc_light::path& );
      friend bool operator !=( const fc_light::path& p, const fc_light::path& );
      friend bool operator < ( const fc_light::path& p, const fc_light::path& );
      ...
      void         replace_extension( const fc_light::path& e );
      fc_light::path     stem()const;
      fc_light::path     extension()const;
      fc_light::path     filename()const;
      fc_light::path     parent_path()const;
  ...
  class directory_iterator {
    public:
      directory_iterator( const fc_light::path& p );
      directory_iterator();
      ~directory_iterator();

      fc_light::path            operator*()const;
  ...
  class recursive_directory_iterator {
    public:
      recursive_directory_iterator( const fc_light::path& p );
  ...
  class variant;
  void to_variant( const fc_light::path&,  fc_light::variant&  );
  void from_variant( const fc_light::variant& , fc_light::path& );
  ...
  public:
     ...
     const fc_light::path& path() const;
     void remove();
     void release();
  protected:
     typedef fc_light::optional<fc_light::path> path_t;
  ...
  public:
     ...
     temp_file(const fc_light::path& tempFolder = fc_light::temp_directory_path(), bool create = false);
  };
  ...
  public:
     ...
     temp_directory(const fc_light::path& tempFolder = fc_light::temp_directory_path());
  };
...
# define FC_LIGHT_HAS_SIMPLE_FILE_LOCK
#endif
#ifdef FC_LIGHT_HAS_SIMPLE_FILE_LOCK
  /** simple class which only allows one process to open any given file. 
   * approximate usage:
   * int main() {
   *   fc_light::simple_file_lock instance_lock("~/.my_app/.lock");
...
#endif // FC_LIGHT_HAS_SIMPLE_FILE_LOCK
