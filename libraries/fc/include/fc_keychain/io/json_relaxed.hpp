...
//#include <fc_keychain/io/buffered_iostream.hpp>
...
namespace fc_keychain { namespace json_relaxed
{
   ...
   fc_keychain::string tokenFromStream( T& in )
   {
      ...
      FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "while parsing token '${token}'",
                                          ("token", token.str() ) );
   }
   ...
   fc_keychain::string quoteStringFromStream( T& in )
   {
       ...
       try
       {
           ...
           switch( q )
           {
               case '\'':
                   if( strict )
                       FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: '\"' at beginning of string, got '\''" );
                   // falls through
               case '"':
                   break;
               default:
                   if( strict )
                       FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: '\"' at beginning of string" );
                   else
                       FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: '\"' | '\\\'' at beginning of string" );
           }
           if( in.peek() == q )
           {
               ...
               if( strict )
                   FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "triple quote unsupported in strict mode" );
               else
               {
                   in.get();
                   ...
                   while( true )
                   {
                       ...
                       if( c == q )
                       {
                           ...
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "unexpected EOF in string '${token}'",
                                      ("token", token.str() ) );
           ...
           while( true )
           {
               ...
               if( c == q )
               {
                   ...
                   FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "unexpected EOL in string '${token}'",
                              ("token", token.str() ) );
               else
               {
                   in.get();
   ...
   fc_keychain::string stringFromStream( T& in )
   {
      try
      {
         ...
         switch( c )
         {
             ...
             case 'r':
                 if( strict )
                     FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "raw strings not supported in strict mode" );
             case 'R':
                 in.get();
                 c2 = in.peek();
                 switch( c2 )
                 {
                     ...
                     default:
                         if( strict )
                             FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "unquoted strings not supported in strict mode" );
             ...
             default:
                 FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: string" );
         }

       } FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "while parsing string" );
   }
   ...
   fc_keychain::variant parseInt( const fc_keychain::string& token, size_t start )
   {
       ...
       if( i >= n )
           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "zero-length integer" );
       ...
       while(true)
       {
           ...
           uint8_t vc = ctbl[c];
           if( vc == 0xFF )
               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal character {c} in integer of base {b}", ("c", c)("b", base) );
           if( val > maxb4mul )
               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "integer literal overflow" );
           val *= base;
       ...
       if( token[0] == '-' )
       {
           if( val > INT64_MAX_PLUS_ONE )
               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "negative integer literal overflow" );
   ...
   { try {
       ...
       if( n == 0 )
           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: non-empty token, got: empty token" );
       switch( token[0] )
       {
           case '+':
               if( strict )
                   FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "unary + not supported in strict mode" );
               i++;
               break;
           case '-':
               i++;
       ...
       switch( c )
       {
           case '0':
               ...
               switch( token[i] )
               {
                   ...
                   case 'B':
                       if( strict )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "binary numeric literals not supported in strict mode" );
                       i++;
                       if( i >= n )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "empty binary numeric literal" );
                       return maybeParseInt<strict, 2>( token, i+1 );
                   case 'o':
                   case 'O':
                       if( strict )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "octal numeric literals not supported in strict mode" );
                       return maybeParseInt<strict, 8>( token, i+1 );
                   case 'x':
                   case 'X':
                       if( strict )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "hex numeric literals not supported in strict mode" );
                   ...
                   default:
                       ...
                       if( strict )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected '.'|'e'|'E' parsing number, got '{c}'",
                               ( "c", c ) );
               }
               break;
           ...
           case '_': case '-': case '.': case '+': case '/':
               if( strict )
                   FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' parsing number", ( "c", c ) );
               return fc_keychain::variant( token );
           default:
               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in token", ( "c", c ) );
       }
       size_t start = i-1;
           ...
           switch( c )
           {
               ...
               case '.':
                   ...
                   if( dot_ok )
                   {
                       ...
                       {
                           if( strict )
                               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "number cannot end with '.' in strict mode" );
                       ...
                       switch( c )
                       {
                           ...
                           case 'E':
                               if( strict )
                                   FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected digit after '.'" );
                               break;
                           ...
                           case '_': case '-': case '.': case '+': case '/':
                               ...
                               return fc_keychain::variant( token );
                           default:
                               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in token", ( "c", c )("i",int(c)) );
                       }
                   }
                   else
                   {
                       if( strict )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal multiple . in number" );
                       return fc_keychain::variant( token );
                   }
                   break;
               case 'e':
               case 'E':
                   ...
                   {
                       if( strict )
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected exponent after 'e'|'E' parsing number" );
                       return fc_keychain::variant( token );
                   }
                   c = token[i++];
                   switch( c )
                   {
                       case '+': case '-':
                           ...
                           {
                               if( strict )
                                   FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected exponent" );
                               return fc_keychain::variant( token );
                           }
                           break;
                       case '0': case '1': case '2': case '3': case '4':
                       ...
                       case '_':           case '.':           case '/':
                           if( strict )
                               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in number", ( "c", c ) );
                           return fc_keychain::variant( token );
                       default:
                           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "illegal character '{c}' in token", ( "c", c ) );
                   }
   ...
   } FC_KEYCHAIN_CAPTURE_AND_RETHROW( (token) ) }
   ...
   variant_object objectFromStream( T& in )
   {
      ...
      try
      {
         char c = in.peek();
         if( c != '{' )
            FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception,
         ...
         while( in.peek() != '}' )
         {
            ...
            if( in.peek() != ':' )
            {
               FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Expected ':' after key \"${key}\"",
                                        ("key", key) );
            }
            in.get();
         ...
         FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Expected '}' after ${variant}", ("variant", obj ) );
      }
      catch( const fc_keychain::eof_exception& e )
      {
         FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Unexpected EOF: ${e}", ("e", e.to_detail_string() ) );
      }
      catch( const std::ios_base::failure& e )
      {
         FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Unexpected EOF: ${e}", ("e", e.what() ) );
      } FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "Error parsing object" );
   }
   ...
   variants arrayFromStream( T& in )
   {
      ...
      {
        if( in.peek() != '[' )
           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Expected '['" );
        in.get();
        skip_white_space(in);
        ...
        if( in.peek() != ']' )
           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Expected ']' after parsing ${variant}",
                                    ("variant", ar) );

        in.get();
      } FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "Attempting to parse array ${array}",
                                         ("array", ar ) );
      return ar;
   }
   ...
   { try {
       ...
       if( strict && !(result.is_int64() || result.is_uint64() || result.is_double()) )
           FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: number" );
       return result;
   } FC_KEYCHAIN_CAPTURE_AND_RETHROW() }
   ...
   variant wordFromStream( T& in )
   {
       ...
       FC_KEYCHAIN_ASSERT( token.length() > 0 );
       ...
       FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "expected: null|true|false" );
   }
   ...
   variant variant_from_fname_from_stream( T& in )
   {
       ...
       try
       {
           ...
           {
               FC_KEYCHAIN_ASSERT( static_cast<uint32_t>( fdata.size() ) == fdata.size() );
               return variant( to_hex( fdata.data(), fdata.size() ) );
           }
           else // '<'
               return variant( std::move( fdata ) );
       }
       FC_KEYCHAIN_RETHROW_EXCEPTIONS( warn, "Atempting to use @fname for ${fname}", ("fname", fname ) );
   }
   ...
   variant variant_from_stream( T& in )
   {
      ...
      while( signed char c = in.peek() )
      {
         switch( c )
         {
            ...
            case EOF:
              FC_KEYCHAIN_THROW_EXCEPTION( eof_exception, "unexpected end of file" );
            case '@':
            case '<':
            case '%':
                if(cmdline)
                ...
                // !fallthrough
            default:
              FC_KEYCHAIN_THROW_EXCEPTION( parse_error_exception, "Unexpected char '${c}' in \"${s}\"",
