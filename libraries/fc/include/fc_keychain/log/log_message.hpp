...
//        string        get_task_name()const;
   ...
   class log_message
   {
      public:
         log_message();
         /**
          *  @param ctx - generally provided using the FC_KEYCHAIN_LOG_CONTEXT(LEVEL) macro
          */
...
FC_KEYCHAIN_REFLECT_TYPENAME( fc_keychain::log_message );
...
/**
 * @def FC_KEYCHAIN_LOG_CONTEXT(LOG_LEVEL)
 ...
 */
#define FC_KEYCHAIN_LOG_CONTEXT(LOG_LEVEL) \
   fc_keychain::log_context( fc_keychain::log_level::LOG_LEVEL, __FILE__, __LINE__, __func__ )
   
/**
 * @def FC_KEYCHAIN_LOG_MESSAGE(LOG_LEVEL,FORMAT,...)
 *
 ...
 */
#define FC_KEYCHAIN_LOG_MESSAGE( LOG_LEVEL, FORMAT, ... ) \
   fc_keychain::log_message( FC_KEYCHAIN_LOG_CONTEXT(LOG_LEVEL), FORMAT, fc_keychain::mutable_variant_object()__VA_ARGS__ )
